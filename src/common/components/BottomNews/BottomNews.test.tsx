...
describe('<BottomNews />', () => {
  it('should render without crashing', () => {
    render(<BottomNews newsData={dummyNewsData} loading={false} error='' />);
  });

  it('should render correct titles', () => {
    const { getByText } = render(
      <BottomNews newsData={dummyNewsData} loading={false} error='' />
    );
    getByText(
      /RELX: The LexisNexis Owner Is A Stable Company With A Strong Future/
    );
  ...
  it('should truncate titles longer than 120 characters', () => {
    ...
    const { getByText } = render(
      <BottomNews newsData={dummyData} loading={false} error='' />
    );
