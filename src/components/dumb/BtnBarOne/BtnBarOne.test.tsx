...
describe('<BtnBarOne />', () => {
  ...
  it('should render without crashing', () => {
    render(<BtnBarOne btns={btns} setState={() => {}} />);
  });

  it('should render buttons', () => {
    const { getByText } = render(<BtnBarOne btns={btns} setState={() => {}} />);
    getByText('home');
    getByText('about');
    getByText('contact');
  });
  ...
  it('should call setState on button click', () => {
    const fn = jest.fn();
    const { getByText } = render(<BtnBarOne btns={btns} setState={fn} />);
  ...
  it('should use "text" property to update state when there is no "value" property ', () => {
    ...
    const { getByText } = render(
      <BtnBarOne btns={btns} setState={(val) => (state = val)} />
    );
    const btn = getByText('about');
    fireEvent.click(btn);
