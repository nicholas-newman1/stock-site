...
import { dummyOneMonthData } from '../../dummyData';
...
const QuoteChart = ({ symbol }) => {
  ...
  const fetchOneMonthData = async () => {
    ...
    //   `https://financialmodelingprep.com/api/v3/historical-chart/1hour/${symbol}?apikey=${process.env.REACT_APP_FMP_KEY}`
    // );
    // let data = await res.json();

    let data = [...dummyOneMonthData];
