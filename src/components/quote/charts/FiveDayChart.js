...
import { dummyFiveDayData } from '../../dummyData';
...
const QuoteChart = ({ symbol }) => {
  ...
  const fetchFiveDayData = async () => {
    ...
    //   `https://financialmodelingprep.com/api/v3/historical-chart/5min/${symbol}?apikey=${process.env.REACT_APP_FMP_KEY}`
    // );
    // let data = await res.json();

    let data = [...dummyFiveDayData];
